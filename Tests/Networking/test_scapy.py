...
def get_callback(**kwargs):
    defaults = dict(
        options_order=['MSS', 'NOP', 'WS', 'SACK', 'TS'],
        ip_id_random=0,
        tcp_ip_id_zero=0,
    ...
    from Networking.MiniNet.scapydaemon import make_callback
    return make_callback(**defaults)


class TestTCPCallback:

    def test_syn_rewrites_window(self):
        # Arrange
        pkt = IP(src='1.1.1.1', dst='2.2.2.2') / TCP(flags='S', options=[('MSS', 1460)])
    ...
    def test_synack_rewrites_options(self):
        # Arrange
        pkt = IP(src='1.1.1.1', dst='2.2.2.2') / TCP(flags='SA', options=[('MSS', 1460), ('SAckOK', b'')])
        mock_pkt = make_mock_pkt(pkt)
